...
export const analyzeDiagnostic = onRequest({ region: REGION, invoker: "public" }, async (req, res) => {
  ...
  try {
    ...
    **Diagnostic Data:**
    ...
    const analysisText = result.response?.candidates?.[0]?.content?.parts?.map(p => (p as any).text || "").join("\n") || "Analysis generation failed";
 ...
 */
export const sendDiagnosticEmail = onRequest({ region: REGION, invoker: "public" }, async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).send("Method Not Allowed");
    return;
  }
 ...
 */
export const createSubmission = onRequest({ region: REGION, invoker: "public" }, async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).send("Method Not Allowed");
    return;
  }
