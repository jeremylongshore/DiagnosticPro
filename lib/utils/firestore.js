...
export const addDiagnosticSubmission = async (data) => {
    try {
...
export const getUserDiagnostics = async (userId) => {
    try {
        const q = query(collection(db, 'diagnosticSubmissions'), where('userId', '==', userId), orderBy('createdAt', 'desc'));
...
export const updateDiagnosticStatus = async (diagnosticId, status) => {
    try {
        const docRef = doc(db, 'diagnosticSubmissions', diagnosticId);
