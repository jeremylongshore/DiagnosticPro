...
describe('DocumentAttachPanel', () => {
  it('renders the work-order/document explanation and controls', () => {
    render(<DocumentAttachPanel submissionId="sub_1" transport={makeTransport()} />);
  ...
  it('lists only document evidence and exposes OCR status honestly', async () => {
    ...
    render(<DocumentAttachPanel submissionId="sub_1" transport={transport} />);
  ...
  it('uploads a work order with its kind and filename', async () => {
    ...
    expect(transport.upload).toHaveBeenCalledWith('sub_1', expect.anything(), 'work-order.pdf', 'work_order');
